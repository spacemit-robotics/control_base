...
static int send_velocity_cmd(struct chassis_dev *dev) {
    ...
    float v_l = vx - wz * wheel_base / 2.0f;
...
static int send_config_cmd(struct chassis_dev *dev) {
    struct rpmsg_esos_priv *priv = dev->priv_data;
    char cmd[96];
    int len;
    ...
    len = snprintf(cmd, sizeof(cmd), "CFG,%.3f,%.3f,%.3f,%.3f,%.3f,%d",
            priv->reduction_ratio, priv->ff_factor, priv->pid_kp,
            priv->pid_ki, priv->pid_kd, priv->feedback_enable ? 1 : 0);
    if (len <= 0)
        return -1;

    if (write(priv->rpmsg_fd, cmd, len + 1) < 0) {
        printf("[CHASSIS-RPMSG-ESOS] Write CFG error: %s\n", strerror(errno));
        return -1;
    }

    printf("[CHASSIS-RPMSG-ESOS] Sent CFG command: %s\n", cmd);
    return 0;
}
...
static int rpmsg_esos_start(struct chassis_dev *dev) {
    ...
    if (dev->priv_data) {
        struct rpmsg_esos_priv *priv = dev->priv_data;
        if (priv->cfg_send_on_startup && !priv->cfg_sent) {
            if (send_config_cmd(dev) == 0)
                priv->cfg_sent = true;
        }
    }
...
rpmsg_esos_create(const char *name, const struct chassis_config *config) {
    ...
    priv->remote_addr = rpmsg_config->remote_addr ?
        ...
        priv->reduction_ratio = rpmsg_config->reduction_ratio;
        priv->ff_factor = rpmsg_config->ff_factor;
        priv->pid_kp = rpmsg_config->pid_kp;
        priv->pid_ki = rpmsg_config->pid_ki;
        priv->pid_kd = rpmsg_config->pid_kd;
        priv->cfg_send_on_startup = rpmsg_config->cfg_send_on_startup;
        priv->feedback_enable = rpmsg_config->feedback_enable;
        priv->cfg_sent = false;
